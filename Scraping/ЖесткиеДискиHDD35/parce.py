...
def get_api_id():
    url = "https://www.dns-shop.ru/search/?q=%D0%B6%D0%B5%D1%81%D1%82%D0%BA%D0%B8%D0%B5+%D0%B4%D0%B8%D1%81%D0%BA%D0%B8+3.5%22&category=17a8914916404e77&stock=now"
    response = requests.get(url, headers=headers, cookies=cookies)

    with open("Scraping//ЖесткиеДискиHDD35//index.html", "w", encoding="utf-8") as file:
        file.write(response.text)

    with open("Scraping//ЖесткиеДискиHDD35//index.html", encoding="utf-8") as file:
        page = file.read()
    ...
    url = f"https://www.dns-shop.ru/search/?q=%D0%B6%D0%B5%D1%81%D1%82%D0%BA%D0%B8%D0%B5+%D0%B4%D0%B8%D1%81%D0%BA%D0%B8+3.5%22&category=17a8914916404e77&stock=now"
...
def data_write_json_csv(link_api):
    ...
    for ind, item in enumerate(link_api):
        ...
        with open("Scraping//ЖесткиеДискиHDD35//api.json", "w", encoding="utf-8") as file:
            json.dump(response.json(), file, indent=4, ensure_ascii=False)

        with open("Scraping//ЖесткиеДискиHDD35//api.json", encoding="utf-8") as file:
            js = json.load(file)
    ...
    with open(f"Scraping//ЖесткиеДискиHDD35//result.json", "w", encoding="utf-8") as file:
