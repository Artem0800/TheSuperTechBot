...
def get_api_id():
    url = "https://www.dns-shop.ru/search/?q=%D0%BE%D1%85%D0%BB%D0%B0%D0%B6%D0%B4%D0%B5%D0%BD%D0%B8%D0%B5&category=17a9cc2d16404e77&stock=now"

    response = requests.get(url, headers=headers, cookies=cookies)

    with open("Scraping//КулерыДляПроцессоров//index.html", "w", encoding="utf-8") as file:
        file.write(response.text)

    with open("Scraping//КулерыДляПроцессоров//index.html", encoding="utf-8") as file:
        page = file.read()
    ...
    for item in range(1, pagination_count + 1):
        url = f"https://www.dns-shop.ru/search/?q=%D0%BE%D1%85%D0%BB%D0%B0%D0%B6%D0%B4%D0%B5%D0%BD%D0%B8%D0%B5&category=17a9cc2d16404e77&stock=now&p={item}"
...
def data_write_json_csv(link_api):
    ...
    for ind, item in enumerate(link_api):
        ...
        with open("Scraping//КулерыДляПроцессоров//api.json", "w", encoding="utf-8") as file:
            json.dump(response.json(), file, indent=4, ensure_ascii=False)

        with open("Scraping//КулерыДляПроцессоров//api.json", encoding="utf-8") as file:
            js = json.load(file)
    ...
    with open(f"Scraping//КулерыДляПроцессоров//result.json", "w", encoding="utf-8") as file:
