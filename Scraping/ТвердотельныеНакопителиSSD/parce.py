...
def get_api_id():
    url = "https://www.dns-shop.ru/search/?q=ssd-%D0%BD%D0%B0%D0%BA%D0%BE%D0%BF%D0%B8%D1%82%D0%B5%D0%BB%D1%8C&category=8a9ddfba20724e77&stock=now"

    response = requests.get(url, headers=headers, cookies=cookies)

    with open("Scraping//ТвердотельныеНакопителиSSD//index.html", "w", encoding="utf-8") as file:
        file.write(response.text)

    with open("Scraping//ТвердотельныеНакопителиSSD//index.html", encoding="utf-8") as file:
        page = file.read()
    ...
    for item in range(1, pagination_count + 1):
        url = f"https://www.dns-shop.ru/search/?q=ssd-%D0%BD%D0%B0%D0%BA%D0%BE%D0%BF%D0%B8%D1%82%D0%B5%D0%BB%D1%8C&category=8a9ddfba20724e77&stock=now&p={item}"
...
def data_write_json_csv(link_api):
    ...
    for ind, item in enumerate(link_api):
        ...
        with open("Scraping//ТвердотельныеНакопителиSSD//api.json", "w", encoding="utf-8") as file:
            json.dump(response.json(), file, indent=4, ensure_ascii=False)

        with open("Scraping//ТвердотельныеНакопителиSSD//api.json", encoding="utf-8") as file:
            js = json.load(file)
    ...
    with open(f"Scraping//ТвердотельныеНакопителиSSD//result.json", "w", encoding="utf-8") as file:
